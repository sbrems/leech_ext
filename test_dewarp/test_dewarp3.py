...
xt=np.polynomial.polynomial.polygrid2d(range(1024),range(1024),np.array(Kx).T)
yt=np.polynomial.polynomial.polygrid2d(range(1024),range(1024),np.array(Ky).T)

#dim=dewarp(image,np.array(Kx).T,np.array(Ky).T,order=0)
tt=datetime.datetime.now()
dim=map_coordinates(image,[yt,xt],order=3)
...
jFits.pyfits.writeto('python_dewarped3_grid.fits',dim,clobber=True)
